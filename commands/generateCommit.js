  ...
  handler: async (argv) => {
    ...
    try {
      ...
      if (!pushCommand) {
        ...
        console.log(
          "\x1b[33m%s\x1b[0m",
          "Info: No Git remote detected. Commits will still be generated, but pushing is not possible."
        );

        if (argv.push) {
          console.log(
            "\x1b[33m%s\x1b[0m",
            "Warning: The --push flag was provided, but will be ignored due to the absence of a Git remote."
          );
        }
      }
      ...
      if (!status) {
        console.log(
          "\x1b[33m%s\x1b[0m",
          "No files to commit. Please stage some files first."
        );
        return;
      }
      ...
      if (committedFiles > 0) {
        if (pusable && pushCommand) {
          ...
          if (shouldPush) {
            ...
            console.log(
              "\x1b[33m%s\x1b[0m",
              "Push ignoré. Les commits sont sauvegardés localement."
            );
          }
        } else {
          console.log(
            "\x1b[33m%s\x1b[0m",
            "Aucun dépôt distant (remote) n'a été détecté. Le push est ignoré."
          );
        }
      }
      ...
      console.log(
        "\x1b[32m%s\x1b[0m",
        "Git repository detected. Ready to generate a commit!"
      );
    } catch (error) {
